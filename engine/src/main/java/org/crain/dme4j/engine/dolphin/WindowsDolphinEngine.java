...
import org.crain.dme4j.engine.dolphin.platform.windows.WindowsKernel;
...
import static org.crain.dme4j.engine.dolphin.platform.windows.WindowsKernel.*;
import static org.crain.dme4j.engine.dolphin.platform.windows.WindowsProcessMemory.*;
...
class WindowsDolphinEngine extends AbstractDolphinMemoryEngine {
    ...
    public void connect(ConnectionDetails connectionDetails) {
        ...
        if (m_hDolphin > 0) {
            obtainEmuRAMInformation(); // Need to actually validate the response.
        }
    }
    ...
    public boolean getStatus() {
        return false; // Currently unused. Should be a handle
    }

    @Override
    public byte[] readFromRAM(final long consoleAddress, final int size) throws DmeCoreModuleException {
    ...
    @Override
    public boolean writeToRAM(final long consoleAddress, byte[] val) throws DmeCoreModuleException {
    ...
    long getRAMAddress(final long consoleAddress) {
        ...
        LOGGER.atError().setMessage("WHAT? 0x{}")
                ...
                .log();
        // Need to remember if this is even required. I think m_ARAMAccessible is always false right now
//        if (m_ARAMAccessible) {
...
//        }
    ...
    private boolean obtainEmuRAMInformation() { // I wonder if this can be reduced in nested.
